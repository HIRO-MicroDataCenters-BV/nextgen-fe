...
export type JsonLdTransformDeps = {
    getFieldDefinition: (key: string, context: 'dataset' | 'distribution') => FieldDefinition | undefined;
    distributionSchema: Record<string, FieldDefinition>;
    generateDatasetId: () => string;
    generateDistributionId: (datasetId: string, index: number) => string;
    generateId: IdFactory;
};

export function createJsonLdTransform(deps: JsonLdTransformDeps) {
    const {
        getFieldDefinition,
        distributionSchema,
        generateDatasetId,
